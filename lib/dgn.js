...
for(var k in o)
 ...
 exports.transformJson=function(TransformJsonType,results,apiAction)
 {
    var resultsJsonObject={"returnCode":0};
   if (TransformJsonType=='SQL')
     {
       resultsJsonObject.items=results;
       return resultsJsonObject;
     }
    else if (TransformJsonType=='FORMLIST' && apiAction=='READ' )  // 自动生成读列表SQL语句  固定会有两条SQL语句（第一条为列表总记录数）
                {
    ...
    else if (TransformJsonType=='FORMLIST' && apiAction=='EXPORT' )  // 自动生成导出列表SQL语句
                {
                  resultsJsonObject.items=results;
                  return resultsJsonObject;
                }
    else if (TransformJsonType=='FORM' && apiAction=='READ' )  // 自动生成读SQL语句
          {
            resultsJsonObject.items={};
    ...
    else if (TransformJsonType=='FORM' && apiAction=='SAVE')  // 自动生成保存SQL语句
       {
         resultsJsonObject.items=[{result:"success",resultDescribe:"保存成功"}];
         return resultsJsonObject;
       }
    else if (TransformJsonType=='FORM' && apiAction=='DELETE')  // 自动生成删除SQL语句
       {
