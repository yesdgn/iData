...
for(var k in o)
 ...
 exports.transformJson=function(ApiType,results)
 {
   ...
   if (ApiType=='SQL')
   ...
   else if (ApiType=='TREE')  // 返回树对象
     {
       ...
       {result=results;}
       buildTreeJson(result,0,treeJson);
...
exports.queryFormat=function (sqlstr, params) {
  let sqlreplace=sqlstr.replace(/\{\$req.dgnFilter\}/gi, params==''?'':'1'); //将req.dgnFilter重置，没有过滤条件则为空 有过滤则为1
      sqlreplace=  sqlreplace.replace(/\{\$req.json.(.*?)\}/gi, function (txt, key) {
    if (Object.prototype.hasOwnProperty.call(params, key)) {
      if (params[key]=='')     
      {return 'null'; }
      else {
         if (key=='sqlstr') {return  params[key] ;}  
         else { return replaceFilterStr(params[key]);
         }
       }
    }
    return '';
